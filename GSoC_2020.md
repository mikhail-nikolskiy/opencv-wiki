...
1. #### _IDEA:_ Better SIFT in the main repository
   * ***Description:*** In 2020 March the patent on one of the most popular feature detection algorithm, SIFT, expires. So, we can move the implementation from opencv_contrib/xfeatures2d to the main OpenCV repository (opencv/features2d) in the late spring or summer. We can also optimize it further and improve it further, probably create bit-exact implementation.
   * ***Expected Outcomes:*** 
      ...
      * Add A-SIFT into the C++ interface. Right now there is Python script to compute A-SIFT on top of SIFT. It can be put to C++ for better efficiency (including parallel processing of the transformed images).
  * ***Resources:***
