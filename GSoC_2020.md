...
1. #### _IDEA:_ Optimize OpenCV for RISC-V
   * ***Description:*** OpenCV provides a convenient method to port many optimized kernels at once to a new CPU, as long as that CPU supports SIMD instructions. We use so-called *Wide Universal Intrinsics* for that. By adding yet another implementation of the wide universal intrinsics for RISC-V we can make OpenCV run pretty efficiently on RISC-V CPUs.
   * ***Expected Outcomes:*** 
      * intrin_riscv.hpp added to the `opencv_core` module. It does not have to implement all of the intrinsics. We can start with vector single-precision floating-point arithmetics.
      * tweaking OpenCV CMake scripts to allow for cross-compiling OpenCV for RISC-V (Linux OS).
...
flow algorithms.
   ...
   * ***Skills Required:*** mastery plus experience coding in C++; basic skills in optimizing code using SIMD.
   * ***Mentors:*** < your name goes here >
   * ***Difficulty:*** Hard
