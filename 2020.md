...
* Intel support for embedded to use
   * Regular build for ARM, Rasberry Pi OpenVino+OpenCV
   * Keem Bay chip
* Model Zoo
   * OpenZoo open
   * Intel Model zoo -- more optimized for Intel chipset
   * https://github.com/opencv/open_model_zoo/tree/master/models
      * DNN with OpenVino backend, runs both Intel NGraph
      * DNN compiled w/o runs only the ONNX
      * Hard to know what OpenVino accelerator doesn't cover

### *__To Dos__*
* All
  - [ ] sign up mentors and get students
* Gary
  - [ ] Ask Stephen Li admin@opencv.org associated phone number not Satya's
