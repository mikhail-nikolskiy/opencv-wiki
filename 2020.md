...
* Vadim:
   * calib3d is split into 3 modules; pull request has been merged (#18940). We can now add the stuff from opencv_contrib.
...
* Anna:
   * opencv ci in china: docker images are prepared for testing (ubuntu, w10, ...). Deployed Jenkins-CI server and agents (Ubuntu, Ubuntu ARM, Windows 10) for OpenCV China. Prepared Docker images for CI build and test (Ubuntu 18.04, Ubuntu 20.04, Ubuntu 18.04 ARM). Work on CI pipelines is in progress. 
   * opencv python module activities: 1. study the existing pip package, 2. improve upon it
   * better video capture tests: Introduced tests for cv::CAP_PROP_FRAME_MSEC in cv::VideoCapture (PR #18968), helped to finalize related PR #18966. 
   * fixing dnn vulkan backend
   * Reviewed, tested, helped to resolve issues with PRs #18955, #18971, 18948
   * Sergei: Changed current implementation of downloading script
   * bilinear layer in deep learning
* Gary:
