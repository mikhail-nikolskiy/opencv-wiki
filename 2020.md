...
* GSoC
   ...
   * Julia bindings pull request in
       * Might make 4.4.0 release
       * 3 stage project 
          * Docs, plans DNN support, automatic generation of bindings 
   * RANSAC 
...
* Chinese "GSoC"
   ...
   * Macbeth project -- connect thim
* Ficus -- compiler runs now
* Webinar TenCent
   * Series of webinars
   * Also wants to be on OpenCV channel
* OpenAI lab (Shenzhen)
   * Tengine (focus on performance on ARM -- multi threaded)
   * C++ to C for better performance
   * Upgrade contracts from OpenCV.cn to OpenCV.org partner program
   * Courseware OpenCV on Edge (hands on -- sell board raspberry PI + OpenCV) w/there name
* Arm servers needed
   * They are expensive
* RiskV servers
