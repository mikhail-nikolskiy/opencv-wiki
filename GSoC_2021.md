...
1. #### _IDEA:_ Implement samples for OpenCV 3D module using OpenGL and GLFW
   * ***Description:*** The newly created OpenCV 3D module includes some interesting functionality that needs some cool samples for demonstration. In OpenCV we have `opencv_vis` module, but has quite heavy `VTK` dependency that may be unavailable or difficult to install on some platforms. On the other hand, often the basic OpenGL API and some interactive framework on top of it (such as GLFW) are enough to demonstrate 3D object, zoom/rotate them etc.
   * ***Expected Outcomes:*** 
...
1. #### _IDEA:_ Extend the support Audio IO module
